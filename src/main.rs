...
fn main() -> WindyResult<()> {
    ...
    let is_tray_mode = preliminary_args.as_ref().map_or(false, |a| a.tray_mode_active);
    let inheriting_console = is_inheriting_console();

    // Relaunch in detached tray mode if needed
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        let msg = format!(
            ...
            panic_info.location().map(|x| x.to_string()).unwrap_or_else(|| "unknown location".to_string())
        );
        eprintln!("{}", msg);
        tracing::error!("{}", msg);
    ...
    info!(
        ...
        ran_from_inherited_console, args.tray_mode_active
    );
